...
public class MyExpressionParser2 {
	...
	public String evaluate(String expStr) throws Exception {
		...
		try {
			result = this.parseAddOrSub(); // 计算加减法表达式
		} catch (Exception e) {
			//除数为0, 忽略异常, 返回0;
		...
		// 计算完加减乘除表达式，应该就是表达式结束符，如果不是，则返回异常
		if (!this.token.equals(EOE)) {
	 ...
	 * 计算方法运算
	 */
	protected double parseFunction() throws Exception {
		double result; // 结果
		// 如果当前标记为左括号，则表示是一个括号运算
		if (this.token.equals("(")) {
			this.getToken(); // 取下一标记
	...
	public static void main(String[] args) throws Exception {
		MyExpressionParser2 parser = new MyExpressionParser2();
