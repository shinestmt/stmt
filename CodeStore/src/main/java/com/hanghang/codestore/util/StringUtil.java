			...
			if(c == '{'){
				...
				if(started){
					...
					try {
						...
						started = false;	//匹配占位符结束
						continue;
					} catch (NumberFormatException e) {
						throw Lang.handException("非法占位符{%s}", result.substring(start+1, end));
					} catch (ArrayIndexOutOfBoundsException e) {
						throw Lang.handException(e, "表达式%s缺少参数{%s}", exp, result.substring(start+1, end));
					}
				}
			}
			i++;
