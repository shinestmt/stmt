 ...
 * Contact: <a
 * href="mailto:emmanuel.alliel@gmail.com">emmanuel.alliel@gmail.com</a><br>
...
public class Jtpl {
	...
	// flag for backwards compatibility, will probably be reomved in future
	// releases
	private boolean failSilently = false;
	private boolean implicitMain = false;

	/**
	 * Constructs a Jtpl object and reads the template from a file. For
	 * backwards compatibility this constructor enables 'failSilently'.
	 * 
	 * @param fileName
	 *            the <code>file name</code> of the template, exemple:
	 *            "java/folder/index.tpl"
	 * @throws IOException
	 *             when an i/o error occurs while reading the template.
	 */
	public Jtpl(String fileName) throws IOException {
		this(new File(fileName));
		// this is the old constructor so it enables the old (pre-1.3) behavior
		// on errors
	...
	/**
	 * Constructs a Jtpl object and reads the template from arbitrary input
	 * source.
	 * 
	 * @param template
	 *            the template source
	...
	/**
	 * Assign a template variable. For variables that are used in blocks, the
	 * variable value must be set before <code>parse</code> is called.
	 * 
	 * @param varName
	...
	/**
	 * Parse a template block. If the block contains variables, these variables
	 * must be set before the block is added. If the block contains subblocks,
	 * the subblocks must be parsed before this block.
	 * 
	 * @param blockName
	 *            the name of the block to be parsed.
	 * @throws IllegalArgumentException
	 *             if the block name is not found (and failSiletly==false)
	 */
	public void parse(String blockName) throws IllegalArgumentException {
		String copy = "";
		if (implicitMain && !"main".equals(blockName)
				&& !blockName.startsWith("main.")) {
		...
		try {
			...
			if (!this.failSilently)
				throw new IllegalArgumentException("Block '" + blockName
						+ "' not found." + " Matches " + locateBlock(blockName));
		}
			...
			if (matcher2.find()) {
				if (parsedBlocks.containsKey(matcher2.group(1))) {
					copy = copy.replaceFirst("\\{" + match + "\\}",
							escape(parsedBlocks.get(matcher2.group(1))
									.toString()));
				} else {
					copy = copy.replaceFirst("\\{" + match + "\\}", "");
				}
				...
				if (vars.containsKey(match)) {
					copy = copy.replaceFirst("\\{" + match + "\\}", escape(vars
							.get(match).toString()));
				} else {
	...
	/**
	 * Template parsing uses regex replace to insert result text, which means
	 * that special characters in replacement string must be escaped.
	 * 
	 * @param replacement
	 ...
	 * @return Blocks where blockName is the child (the Set's toString lists the
	 *         full names)
	 */
	...
	private void makeTree(String fileText) {
		// BEGIN: implicit main
		if (!Pattern
				.compile(".*<!--\\s*BEGIN\\s*:\\s*main\\s*-->.*",
						Pattern.DOTALL).matcher(fileText).matches()) {
		...
		String parentName = "";
		int lastlength = 0; // used in newline trimming to see if one block
		...
		while (matcher.find()) {
			...
			if (matcher.group(1).toUpperCase().equals("BEGIN")) {
				...
				if (blocks.containsKey(currentBlockName)) {
					blocks.put(currentBlockName, blocks.get(currentBlockName)
							+ after);
				} else {
					blocks.put(currentBlockName, after);
				}
				if (blocks.containsKey(parentName)) {
					blocks.put(parentName, blocks.get(parentName) + "{_BLOCK_."
							+ currentBlockName + "}");
				} else {
