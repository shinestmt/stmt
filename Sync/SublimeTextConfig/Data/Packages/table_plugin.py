...
class AbstractTableCommand(sublime_plugin.TextCommand):
    ...
    @property
    def syntax(self):
        syntax_name = self.view.settings().get("table_editor_syntax") # 配置文件获取默认语法 Preferences.sublime-settings
...
class TableEditorAlignCommand(AbstractTableMultiSelect):
    """
    Key: ctrl+shift+a                                                # 快捷键: ctrl+shift+a
    Re-align the table without change the current table field.       # 重新排列, 不改变当前表格的字段
    Move cursor to begin of the current table field.                 # 移动光标到当前单元格的起始位置
    """
...
class TableEditorNextField(AbstractTableMultiSelect):
    """
    Key: tab                                          # 快捷键: tab
    Re-align the table, move to the next field.       # 重新排列, 编辑下一个字段
    Creates a new row if necessary.                   # 如果必须则重启一行
    """
...
class TableEditorPreviousField(AbstractTableMultiSelect):
    """
    Key: shift+tab                                     # 快捷键: shift+tab
    Re-align, move to previous field.                  # 重新排列, 移动光标到前一个当前单元格
    """
...
class TableEditorNextRow(AbstractTableMultiSelect):
    """
    Key: enter                                                      # 快捷键: shift+tab
    Re-align the table and move down to next row.                   # 重新排列, 移动光标到下一行
    Creates a new row if necessary.                                 # 如果必须则重启一行
...
class TableEditorMoveColumnLeft(AbstractTableMultiSelect):
    """
    Key: alt+left                             # 快捷键: alt+left
    Move the current column right.            # 当前列向左移动
    """
...
class TableEditorMoveColumnRight(AbstractTableMultiSelect):
    """
    Key: alt+right                         # 快捷键: alt+right
    Move the current column right.         # 当前列向右移动
    """
...
class TableEditorDeleteColumn(AbstractTableMultiSelect):
    """
    Key: alt+shift+left                  # 快捷键: alt+shift+left
    Kill the current column.             # 删除当前列
    """
...
class TableEditorInsertColumn(AbstractTableMultiSelect):
    """
    Keys: alt+shift+right                                           # 快捷键: alt+shift+right
    Insert a new column to the left of the cursor position.         # 左边新建空白列
    """
...
class TableEditorKillRow(AbstractTableMultiSelect):
    """
    Key : alt+shift+up                                    # alt+shift+up
    Kill the current row or horizontal line.              # 删除当前行, 下方数据上移
    """
...
class TableEditorInsertRow(AbstractTableMultiSelect):
    """
    Key: alt+shift+down                           # alt+shift+down
    Insert a new row above the current row.       # 新增一行, 当前行及以下数据下移
    """
...
class TableEditorMoveRowUp(AbstractTableCommand):
    """
    Key: alt+up                       # 
    Move the current row up.          # 与系统ctrl+shift+up一致, 当前数据与上一行数据交换
    """
...
class TableEditorMoveRowDown(AbstractTableCommand):
    """
    Key: alt+down
    Move the current row down.       # # 与系统ctrl+shift+down一致, 当前数据与下一行数据交换
    """
...
class TableEditorInsertSingleHline(AbstractTableMultiSelect):
    """
    Key: ctrl+k,-                                           # 
    Insert single horizontal line below current row.        # 添加横向分割线(-)
    """
...
class TableEditorInsertDoubleHline(AbstractTableMultiSelect):
    """
    Key: ctrl+k,=
    Insert double horizontal line below current row.        # 添加横向分割线(=)
    """
...
class TableEditorCsvToTable(AbstractTableCommand):
    ...
    Key: ctrl+k, |                                     # 
    Convert selected CSV region into table             # csv转表格
    """
    def run(self, edit):
        new_sels = []
        for sel in self.view.sel():
